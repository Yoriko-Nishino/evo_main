    ...
    def initialize_anchors(self,pi,pj):
        ...
        for item in self.triad_n:
            ...
            if pi in item or pj in item:
                continue
            if (21.6875 in [self.pf_npy[pj][item[0]],self.pf_npy[pj][item[1]],self.pf_npy[pj][item[2]]]):
                continue
            if (21.6875 in [self.pf_npy[pi][item[0]],self.pf_npy[pi][item[1]],self.pf_npy[pi][item[2]]]):
                continue
