    ...
    def initialize_anchors(self,pi,pj):
        ...
        for item in self.triad_n:
            ...
            if pi in item or pj in item:
                continue
            if (threhold in [self.pf_npy[pj][item[0]],self.pf_npy[pj][item[1]],self.pf_npy[pj][item[2]]]):
                continue
            if (threhold in [self.pf_npy[pi][item[0]],self.pf_npy[pi][item[1]],self.pf_npy[pi][item[2]]]):
                continue
            ...
            for idx in range(self.len_stru):
                if idx in item or idx==pi or idx==pj:
                    continue
                if self.pf_npy[pi][idx]==threhold or self.pf_npy[pj][idx]==threhold:
                    continue
                pos4=idx
                anchors=item.copy()
                anchors.append(pos4)
                ...
                if not pos3d:
                    ...
                    sum_anchor.append(anchors)
                    sum_3dpos.append(pos3d)
                    sum_matrix.append(mymatrix)
            ...
            if len(sum_anchor)==0:
                continue
            for anchors,pos3d,mymatrix in zip(sum_anchor,sum_3dpos,sum_matrix):
